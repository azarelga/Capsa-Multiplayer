...
class CapsaHTTPClient:
    ...
    def connect_to_server(self):
        try:
            response = requests.post(
                f"{self.server_url}/api/game", json={"client_name": "Player"}, timeout=5
            )
            ...
            try:
                if self.in_session:
                    response = requests.post(
                        ...
                        json={"client_id": self.client_id, "command": "GET_GAME_STATE"},
                        timeout=5,
                    )
    ...
    def send_command(self, command):
        ...
        try:
            ...
            response = requests.post(
                f"{self.server_url}/api/game", json=command, timeout=10
            )
        ...
        while True:
            ...
            if choice == 1:
                ...
                if (
                    ...
                    and response["command"] == "SESSION_JOINED"
                ):
...
def draw_game(screen, client, WIDTH, HEIGHT):
    ...
    title = font_large.render(
        f"CAPSA MULTIPLAYER HTTP - {client.session_name}", True, WHITE
    )
    ...
    for i, name in enumerate(client.game_data["players_names"]):
        if name:
            ...
            # Highlight only the current player
            ...
            text = font_small.render(
                f"{prefix}Slot {i + 1}: {name} ({count} cards)", True, color
            )
    ...
    if client.game_data["my_hand"]:
        my_cards = [
            CapsaClientCard(card_data) for card_data in client.game_data["my_hand"]
        ]
